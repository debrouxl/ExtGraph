...
| check if tiles need to be animated
    ...
    sub.w    (%a3),%d2
    bne.s    No_anim_DrawAnimatedPlane8B
    ...
    sub.w    (%a3),%d2
    bne.s    RefreshAnimatedBuffer_DrawAnimatedPlane8B
    move.w   %d2,(%a3) | clear step counter
    bra.s    RefreshAnimatedBuffer_DrawAnimatedPlane8B

No_anim_DrawAnimatedPlane8B:
| check if refreshing big_vscreen is necessary
    ...
    bpl.s    dx_pos_DrawAnimatedPlane8B
    neg.w    %d2
dx_pos_DrawAnimatedPlane8B:
    moveq.l  #32,%d4
    cmp.w    %d4,%d2 | old x - new x >= 32
    bge.s    RefreshAnimatedBuffer_DrawAnimatedPlane8B
    ...
    bpl.s    dy_pos_DrawAnimatedPlane8B
    neg.w    %d2
dy_pos_DrawAnimatedPlane8B:
    ...
    eor.w    %d2,%d3
    beq.s    DrawBuffer_DrawAnimatedPlane8B

RefreshAnimatedBuffer_DrawAnimatedPlane8B:
...
DrawBuffer_DrawAnimatedPlane8B:
