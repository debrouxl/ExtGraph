...
extern void FillSpanREVERSE(unsigned short x1 asm("%d0"), unsigned short x2 asm("%d1"), unsigned char * addr asm("%a0"));
...
//        d1=1,d2=0;
        ...
        for (y=y3;y<y2;y++,xa+=m1,xb+=m2,(unsigned char *)addr+=30) {
            FillSpanREVERSE(1+(xa>>7),0+(xb>>7),addr);
        }
...
//        d1=0,d2=1;
        ...
        // now draw the spans
        for (y=y3;y<y2;y++,xa+=m1,xb+=m2,(unsigned char *)addr+=30) {
            FillSpanREVERSE(0+(xa>>7),1+(xb>>7),addr);
        }
        for (;y<y1;y++,xa+=m1,xb+=m3,(unsigned char *)addr+=30) {
            FillSpanREVERSE(0+(xa>>7),1+(xb>>7),addr);
        }
