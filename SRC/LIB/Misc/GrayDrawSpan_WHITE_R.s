| C prototype: void GrayDrawSpan_WHITE_R(unsigned short x1 asm("%d0"), unsigned short x2 asm("%d1"), unsigned char * addrs asm("%a0"));
|
| This routine draws lines from x1 to x2 in line starting at addr, in
| white color. It is primarily intended as callback for Gray(Clip)FilledTriangle_R.
|--------------------------------** WARNING **--------------------------------
...
6:  tst.w    %d2           | if (sx) ...
    ...
    move.w   %d2,%d0
    add.w    %d0,%d0
    move.w   (9b,%pc,%d0.w),%d0 | *p++ ^= table1[sx]
    and.w    %d0,(%a0)+
    and.w    %d0,(%a1)+
    add.w    #-16,%d2
