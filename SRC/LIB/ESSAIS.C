...
// GrayClipSprite32_MASK_R, GrayFastFilledRect_R (Nul).
//Sprite16_MASK ? (geogeo)
// Des X8 ? (Fisch2) 
...
// This test seems strange because the TICT logo is not defined properly:
// the size of an interlaced chunk is 3*8 bytes instead of 3*2 bytes...
/*
GrayOn();

FillScreenWithGarbage_R(1234567891,LCD_SIZE,GrayGetPlane(LIGHT_PLANE));
FillScreenWithGarbage_R(1987654321,LCD_SIZE,GrayGetPlane(DARK_PLANE));
GrayClipISpriteX16_MASK_R(1,1,64,(unsigned short *)mtictlogo64,64/16,GrayGetPlane(LIGHT_PLANE),GrayGetPlane(DARK_PLANE));
ngetchx();
GrayOff();
*/
...
// The VAR-Link refuses archiving it.
