...
#define getpixel8(s,x,y)  (*((unsigned char *)(s)+(y)+((x)>>3)))&bit_table[(x)&7]

//the other routines are similar (the only changes are for the different logics (and extra optimizations for that too))
void ScaleSprite8_AND(const unsigned char *sprite,void *dest,short x0,short y0,short sizex,short sizey) {
    short x;//,y;
    unsigned short u,v=0,du=(unsigned short)(8<<8)/(unsigned short)sizex,dv=(unsigned short)(8<<8)/(unsigned short)sizey,tempv,pix8,pixscreen;
    unsigned char *src;
...
//    for (y = sizey;(y--);) {
    for (sizey = sizey;(sizey--);) {
        ...
        src = dest;
        for (x = sizex;(x--);) {
            pix8=getpixel8(sprite,u>>8,tempv);
            pixscreen=*src&mask;

            if (pix8&&pixscreen) *src|=mask;
            else if (!pix8) *src&=~mask;
