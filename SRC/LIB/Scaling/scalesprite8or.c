...
void ScaleSprite8_OR(const unsigned char *sprite,void *dest,short x0,short y0,short sizex,short sizey) {
    short x;//,y;
    unsigned short u,v=0,du=(unsigned short)(8<<8)/(unsigned short)sizex,dv=(unsigned short)(8<<8)/(unsigned short)sizey,tempv;
    unsigned char *src;
...
//    for (y = sizey;(y--);) {
    for (sizey = sizey;(sizey--);) {
        ...
        src = dest;
        for (x = sizex;(x--);) {
            if (getpixel8(sprite,u>>8,tempv)) *src|=mask;
