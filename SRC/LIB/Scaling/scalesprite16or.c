...
void ScaleSprite16_OR(const unsigned short *sprite,void *dest,short x0,short y0,short sizex,short sizey) {
    short x;//,y;
    unsigned short u,v=0,du=(unsigned short)(16<<8)/(unsigned short)sizex,dv=(unsigned short)(16<<8)/(unsigned short)sizey,tempv;
    unsigned char *src;
...
//    for (y = sizey;(y--);) {
    for (sizey = sizey;(sizey--);) {
        ...
        src = dest;
        for (x = sizex;(x--);) {
            if (getpixel16(sprite,u>>8,tempv)) *src|=mask;
