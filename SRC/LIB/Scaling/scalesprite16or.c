...
/*
void __attribute__((__stkparm__)) ScaleSprite16_OR(unsigned short *sprite,void *dest,short x0,short y0,short sizex,short sizey)
{
    short dx,dy;
    ...
    for(y0=sizey;y0--;)
    {
        ...
        for(x0=sizex;x0--;)
        {
            if(dx>=0) {
                do
                {
                    asm("ror.b #1,%0;bcc.s 0f;addq.l #1,%1;0:" : "=d" (masksrc),"=a"(sprite) : : "cc");
                    dx-=sizex;
                } while(dx>0);
            }

            if(*(unsigned char *)sprite & masksrc) {
                *tmpdest |= mask;
            }
            asm("ror.b #1,%0;bcc.s 0f;addq.l #1,%1;0:":"=d" (mask),"=a" (tmpdest));
            dx+=16;
        }
...
void __attribute__((__stkparm__)) ScaleSprite16_OR(unsigned short *sprite,void *dest,short x0,short y0,short sizex,short sizey)
{
	...
	for(y0=sizey;y0--;)
	{
		...
		for(x0=sizex;x0--;)
		{
			...
			asm("ror.b #1,%0;bcc.s 0f;addq.l #1,%1;0:":"=d" (mask),"=a" (tmpdest):"0"(mask),"1"(tmpdest));
			dx+=16;
		}
		(unsigned char *)dest+=30;
		dy+=16;
