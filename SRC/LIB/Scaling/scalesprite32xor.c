...
void ScaleSprite32_XOR(const unsigned long *sprite,void *dest,short x0,short y0,short sizex,short sizey) {
    short x;//,y;
    unsigned short u,v=0,du=(unsigned short)(32<<8)/(unsigned short)sizex,dv=(unsigned short)(32<<8)/(unsigned short)sizey,pix32,pixscreen,tempv;
    unsigned char *src;
...
//    for (y = sizey;(y--);) {
    for (sizey = sizey;(sizey--);) {
        ...
        src = dest;
        for (x = sizex;(x--);) {
            pix32=getpixel32(sprite,u>>8,tempv);
            pixscreen=*src&mask;

            if (pix32&&pixscreen) *src&=~mask;
            else if (!pix32&&!pixscreen) *src&=~mask;
            else *src|=mask;
