| C prototype: void GraySingleSprite8_WHITE_R(register short x asm("%d0"),register short y asm("%d1"),register short h asm("%d2"),unsigned char *sprt,register void *dest0 asm("%a0"),register void *dest1 asm("%a1")) __attribute__((__stkparm__));
| 
...
.text
.globl GraySingleSprite8_WHITE_R
.even

GraySingleSprite8_WHITE_R:
    dbf      %d2,0f
    rts
