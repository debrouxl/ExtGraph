| C prototype: void GrayClipSingleSprite8_COLOR_R(register short x asm("%d0"),register short y asm("%d1"),register short h asm("%d2"),unsigned char *sprt,unsigned short color,register void *dest0 asm("%a0"),register void *dest1 asm("%a1")) __attribute__((__stkparm__));
| 
...
.text
.globl GrayClipSingleSprite8_COLOR_R
.even

GrayClipSingleSprite8_COLOR_R:
    subq.w   #1,%d2
    blt.s    0f    
