| C prototype: void GrayClipISpriteX16_MASK_R(register short x asm("%d0"), register short y asm("%d1"),register short h asm("%d3"),unsigned short *sprt,register short w asm("%d2"),register void *dest0 asm("%a0"),register void *dest1 asm("%a1")) __attribute__((__stkparm__));
|
| Based on Genlib's put_masked_big_sprite, made by Julien Richard-Foy.
| Modified by Lionel Debroux:
| * converted from A68k assembly to GNU as assembly.
...
.text
.globl GrayClipISpriteX16_MASK_R
.even

GrayClipISpriteX16_MASK_R:
    movem.l  %d3-%d7/%a2,-(%sp)
    move.l   4+24(%sp),%a2
