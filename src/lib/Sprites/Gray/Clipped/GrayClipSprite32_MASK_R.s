| C prototype: void GrayClipSprite32_MASK_R(short x asm("%d0"), short y asm("%d1"), unsigned short height asm("%d2"), const unsigned long *sprt0, const unsigned long *sprt1, const unsigned long *mask0, const unsigned long *mask1, void *dest0 asm("%a0"), void *dest1 asm("%a1")) __attribute__((__stkparm__));

.include "common.s"

.text
.globl GrayClipSprite32_MASK_R
.even
...
10:
    ...
    bhi.w    7b	| x > PLANE_PIXEL_WIDTH-1-32
...
8:
    cmpi.w   #-32,%d0
    ble.s    0b		| x <= -32 ?
| Note: shift count can be large, therefore the shifting may be quite slow...
    neg.w    %d0		| shift = -x
    add.w    %d1,%d1		| %d1 = y*PLANE_BYTE_WIDTH
    adda.w   %d1,%a0		| dest += offset
    adda.w   %d1,%a1
