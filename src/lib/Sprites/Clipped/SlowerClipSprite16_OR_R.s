...
__YPositive_ClipSprite16_OR_R:
    ...
    bhi.s    __End_ClipSprite16_OR_R		| y - (PLANE_PIXEL_HEIGHT-1) > 0 ?
    add.w    %d2,%d3		| %d3 = h + y - (PLANE_PIXEL_HEIGHT-1)
    bmi.s    __CalcOffset_ClipSprite16_OR_R	| h + y - (PLANE_PIXEL_HEIGHT-1) < 0 ?
...
__ClipX_ClipSprite16_OR_R:
    ...
    bhi.s    __ClipXRight_ClipSprite16_OR_R	| x > PLANE_PIXEL_WIDTH-1-15
    lsr.w    #4,%d3		| %d3 = x/16
...
__ClipXRight_ClipSprite16_OR_R:
    ...
    bhi.s    __End_ClipSprite16_OR_R		| x > PLANE_PIXEL_WIDTH-1
