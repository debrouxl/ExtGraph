| C prototype: void SlowerClipSprite8_AND_R(register short x asm("%d0"),register short y asm("%d1"),register short h asm("%d2"),register unsigned char *sprt asm("%a1"),register void *dest asm("%a0"));

.text
.globl SlowerClipSprite8_AND_R
.even

SlowerClipSprite8_AND_R:
    move.w   %d3,-(%sp)

    subq.w   #1,%d2		| %d2 = h - 1
    move.w   %d1,%d3		| %d3 = y
    bpl.s    __YPositive_ClipSprite8_AND_R	| h > 0 ?

    add.w    %d1,%d2		| %d2 = h + y
    bmi.s    __End_ClipSprite8_AND_R		| y + h < 0 ?
...
__ClipX_ClipSprite8_AND_R:
    ...
    moveq    #8,%d1
    sub.w    %d0,%d1
    bge.s    __Loop_ClipSprite8_AND_R_2

__Loop_ClipSprite8_AND_R_1:
    moveq    #-1,%d0
    move.b   (%a1)+,%d0
    swap     %d0
    ror.l    %d1,%d0
...
__Loop_ClipSprite8_AND_R_2:
    ...
    dbf      %d2,_Loop_ClipSprite8_AND_R_2

    move.w   (%sp)+,%d3
    rts

__ClipXLeft_ClipSprite8_AND_R:
    cmpi.w   #-8,%d0
