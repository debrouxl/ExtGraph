| C prototype: void SlowerClipSprite8_XOR_R(register short x asm("%d0"),register short y asm("%d1"),register short h asm("%d2"),register unsigned char *sprt asm("%a1"),register void *dest asm("%a0"));

.text
.globl SlowerClipSprite8_XOR_R
.even

SlowerClipSprite8_XOR_R:
    move.w   %d3,-(%sp)
...
__YPositive_ClipSprite8_XOR_R:
    ...
    bhi.s    __End_ClipSprite8_XOR_R		| y - (PLANE_PIXEL_HEIGHT-1) > 0 ?
    add.w    %d2,%d3		| %d3 = h + y - (PLANE_PIXEL_HEIGHT-1)
    bmi.s    __CalcOffset_ClipSprite8_XOR_R	| h + y - (PLANE_PIXEL_HEIGHT-1) < 0 ?
...
__ClipX_ClipSprite8_XOR_R:
    ...
    bhi.s    __ClipXRight_ClipSprite8_XOR_R	| x > PLANE_PIXEL_WIDTH-1-8
...
__ClipXRight_ClipSprite8_XOR_R:
    ...
    bhi.s    __End_ClipSprite8_XOR_R		| x > PLANE_PIXEL_WIDTH-1
