...
__YPositive_ClipSprite32_OR_R:
    ...
    bhi.s    __End_ClipSprite32_OR_R		| y - 128 > 0 ?
    add.w    %d2,%d3		| %d3 = h + y - 128
    bmi.s    __CalcOffset_ClipSprite32_OR_R	| h + y - 128 < 0 ?
...
__ClipX_ClipSprite32_OR_R:
    ...
    cmpi.w   #239-32,%d0
    bhi.s    __ClipXRight_ClipSprite32_OR_R	| x > 239-32
    lsr.w    #4,%d3		| %d3 = x/16
...
__ClipXRight_ClipSprite32_OR_R:
    cmpi.w   #239,%d0
    bhi.s    __End_ClipSprite32_OR_R		| x > 239
    add.w    %d1,%d1		| %d1 = y*30
    lea.l    26(%a0,%d1.w),%a0
    move.w   %d0,%d3
    andi.w   #15,%d0		| shiftx
