| C prototype: void FastDrawLine(void* plane, unsigned short x1, unsigned short y1, unsigned short x2, unsigned short y2, short mode) __attribute__((__stkparm__));
|
...
| The comments are for the case A_REVERSE is chosen.
    ...
    move.b   %d4,__PutInstr2_FDL__-2f(%a1)      | and.b d6,(a0)
    move.b   %d4,__AttrCenter1_FDL__-2f(%a1)    | and.b d6,(a0)
    addq.b   #2,%d4
    move.b   %d4,__PutInstr1_FDL__+2-2f(%a1)    | and.b d7,(a1)
    move.b   %d4,__PutInstr2_FDL__+2-2f(%a1)    | and.b d7,(a1)
    move.b   (%a2),%d4
    move.b   %d4,__RorInstr_FDL__+2-2f(%a1)     | bcs.s
    move.b   %d4,__RorInstr_FDL__+8-2f(%a1)     | bcs.s
    move.b   %d4,__PutInstr1_FDL__+6-2f(%a1)    | bcs.s
...
| Check if the line is going up or down and patch the code accordingly.
    ...
    bra.s    __FirstDiagonal_FDL__

__LineGoesDownwards_FDL__:
...
__FirstDiagonal_FDL__:
    cmp.w    %d3,%d2
    bls.s    __ScanY_FDL__                  | if dy >= dx
...
__PutInstr1_FDL__:
    ...
    ror.b    #1,%d6                         | X1++
    bcc.s    __Mask1Ok_FDL__
    addq.w   #1,%a0
__Mask1Ok_FDL__:
    rol.b    #1,%d7                         | X2--
    bcc.s    __Mask2Ok_FDL__
    subq.w   #1,%a1
__Mask2Ok_FDL__:
    add.w    %d3,%d1                        |P+=dy
    bmi.s    __SameY_FDL__                  |if P < 0
    sub.w    %d2,%d1                        |P-=dx
    add.w    %d4,%a0                        |Y1++ (or --)
    sub.w    %d4,%a1                        |Y2-- (or ++)
__SameY_FDL__:
    dbf      %d0,__ScanYLoop_FDL__
...
__ScanY_FDL__:                    | The roles of dx and dy are swapped.
    move.w   %d3,%d0
    beq.s    __AttrCenter1_FDL__            |dy = 0 -> only one point.
    move.w   %d3,%d1
    lsr.w    #1,%d1
    ...
    cmp.l    %a1,%a0
    bls.s    __ScanXLoop_FDL__    | When it is in the correct order (Y1 <= Y2)
...
__PutInstr2_FDL__:
    ...
    lea      -30(%a1),%a1                   | Y2--
    add.w    %d2,%d1                        | P+=dx
    bmi.s    __SameX_FDL__                  | if P < 0
    sub.w    %d3,%d1                        | P-=dy
__RorInstr_FDL__:
    ror.b    #1,%d6
    bcc.s    __RorInstr2_FDL__
    addq.w   #1,%a0                         | X1++
__RorInstr2_FDL__:
    rol.b    #1,%d7
    bcc.s    __SameX_FDL__
