| C prototype: void FastLine_Draw_R(void *plane asm("%a0"), unsigned short x1 asm("%d0"), unsigned short y1 asm("%d1"), unsigned short x2 asm("%d2"), unsigned short y2 asm("%d3")) __attribute__((__regparm__));
|
...
XLoop_FL_D_R:
	...
	subq.w	#1,%d0
	bpl.s	Msk1OkX_FL_D_R
	addq.l	#1,%a0
	moveq.l	#7,%d0

Msk1OkX_FL_D_R:
	add.b	%d1,%d1
	bne.s	Msk2OkX_FL_D_R
	subq.l	#1,%a1
	moveq.l	#1,%d1
...
YLoop_FL_D_R:
	...
	sub.w	%d3,%d4	| e -= dy
	subq.w	#1,%d0
	bpl.s	Msk1OkY_FL_D_R
	addq.l	#1,%a0
	moveq.l	#7,%d0

Msk1OkY_FL_D_R:
	add.b	%d1,%d1
	bne.s	Msk2OkY_FL_D_R
	subq.l	#1,%a1
	moveq.l	#1,%d1
Msk2OkY_FL_D_R:
NoIncColY_FL_D_R:
