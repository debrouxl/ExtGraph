...
DrawBufferWithShifts_OR:
    ...
    moveq.l  #127,%d6
LineStart_DrawBufferWithShifts_OR:

    moveq.l  #16,%d1
    move.w   %d7,%d0	| décalage initial
    add.b    (%a2),%d0	| Décalage horizontal
    cmp.w    %d1,%d0	| d0 < 16 ?
    blt.s    OK_DrawBufferWithShifts_OR
    addq.l   #2,%a0
    sub.w    %d1,%d0
OK_DrawBufferWithShifts_OR:
    ...
    add.b    (%a2)+,%d0	| Décalage horizontal
    cmpi.w   #16,%d0	| d0 < 16 ?
    bge.s    OK2_DrawBufferWithShifts_OR
    addq.l   #2,%a0
OK2_DrawBufferWithShifts_OR:

    adda.w   (%a3)+,%a0	| décalage vertical

    dbf      %d6,LineStart_DrawBufferWithShifts_OR
