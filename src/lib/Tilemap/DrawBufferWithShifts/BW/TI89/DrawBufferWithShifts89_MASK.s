	.section .text
	xdef DrawBufferWithShifts89_MASK
...
DrawBufferWithShifts89_MASK:
    ...
    moveq.l  #99,%d6
LineStart_DrawBufferWithShifts89_MASK:

    moveq.l  #16,%d1
    move.w   %d7,%d0	| Décalage initial
    add.b    (%a2),%d0	| Décalage horizontal
    cmp.w    %d1,%d0	| d0 < 16 ?
    blt.s    OK_DrawBufferWithShifts89_MASK
    addq.l   #2,%a0
    sub.w    %d1,%d0
OK_DrawBufferWithShifts89_MASK:
    ...
    add.b    (%a2)+,%d0	| Décalage horizontal
    cmpi.w   #16,%d0	| d0 < 16 ?
    bge.s    OK2_DrawBufferWithShifts89_MASK
    addq.w   #2,%d1
OK2_DrawBufferWithShifts89_MASK:
    ...
    dbf      %d6,LineStart_DrawBufferWithShifts89_MASK
