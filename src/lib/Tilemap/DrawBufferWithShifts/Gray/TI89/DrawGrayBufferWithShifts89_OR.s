	.text
	xdef DrawGrayBufferWithShifts89_OR
...
LineStart_DrawGrayBufferWithShifts89_OR:

    moveq.l  #16,%d1
    move.w   %a6,%d0	| Décalage initial
    add.b    (%a2),%d0	| Décalage horizontal
    cmp.w    %d1,%d0	| d0 < 16 ?
    blt.s    OK_DrawGrayBufferWithShifts89_OR
    addq.l   #2,%a0
    addq.l   #2,%a4
    sub.w    %d1,%d0
OK_DrawGrayBufferWithShifts89_OR:
    ...
    add.b    (%a2)+,%d0	| Décalage horizontal
    cmpi.w   #16,%d0	| d0 < 16 ?
    bge.s    OK2_DrawGrayBufferWithShifts89_OR
    addq.w   #2,%d1
OK2_DrawGrayBufferWithShifts89_OR:
    ...
    dbf      %d7,LineStart_DrawGrayBufferWithShifts89_OR
