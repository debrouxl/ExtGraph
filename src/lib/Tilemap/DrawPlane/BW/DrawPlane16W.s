...
| check if refreshing big_vscreen is necessary
    ...
    bne.s    RefreshBuffer_DrawPlane16W

    move.w   4+2+4+4+4(%a3),%d2 | old x
    move.w   %d2,%d3
    sub.w    %d0,%d2 | old x - new x
    bpl.s    dx_pos_DrawPlane16W
    neg.w    %d2
dx_pos_DrawPlane16W:
    moveq.l  #32,%d4
    cmp.w    %d4,%d2 | old x - new x >= 32
    bge.s    RefreshBuffer_DrawPlane16W
...
dy_pos_DrawPlane16W:
    ...
    eor.w    %d2,%d3
    beq.s    DrawBuffer_DrawPlane16W

RefreshBuffer_DrawPlane16W:
