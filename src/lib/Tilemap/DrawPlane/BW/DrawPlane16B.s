...
| check if refreshing big_vscreen is necessary
    ...
    bne.s    RefreshBuffer_DrawPlane16B

    move.w   4+2+4+4+4(%a3),%d2 | old x
    move.w   %d2,%d3
    sub.w    %d0,%d2 | old x - new x
    bpl.s    dx_pos_DrawPlane16B
    neg.w    %d2
dx_pos_DrawPlane16B:
    moveq.l  #32,%d4
    cmp.w    %d4,%d2 | old x - new x >= 32
    bge.s    RefreshBuffer_DrawPlane16B
...
dy_pos_DrawPlane16B:
    ...
    eor.w    %d2,%d3
    beq.s    DrawBuffer_DrawPlane16B

RefreshBuffer_DrawPlane16B:
